...
const electroreceptor_resting_potential = Float32(-0.06) # -60mV (Corey and Hudspeth 1979)
const electroreceptor_input_resistance  = 2.5e8  # 250Mohm (Corey and Hudspeth 1979)
const electroreceptor_capacitance = 3.0e-11 # 30pF (Roberts, Howard and Hudspeth, 1988)
const electroreceptor_transduction_reversal_potential = Float32(-.002) # -2mv (C&H 1979)
const electroreceptor_single_channel_conductance = Float32(500.e-12) # Geleoc &c 1997
...
"""
function ElectroReceptor(resting_potential)

    p = p_open(0.0)
    return ElectroReceptor([resting_potential],
                    [p],
                    rand(48).<p,
                    [0.0],
                    [0.0],

                    electroreceptor_resting_potential,
                    electroreceptor_input_resistance,
                    electroreceptor_capacitance,
                    48,
                    electroreceptor_single_channel_conductance,
                    electroreceptor_transduction_reversal_potential)
end

"""
  #  Hair cell behaviour
"""
function electroreceptor_stateupdate!(electroreceptor::ElectroReceptor, kinocilium_deflection)
  ...
  electroreceptor.Δk[] = (kinocilium_deflection + 2.0f0*randn(Float32,1)[])*nano

  electroreceptor.p_open[] = p_open(electroreceptor.Δk[])  # gate open probability

  electroreceptor.gateOpen[:] = rand(48) .< electroreceptor.p_open[]     # gate states

  conductance = Float32(sum(electroreceptor.gateOpen))*electroreceptor.conductance

  # leak current
  leak_current = (electroreceptor.potential[] - electroreceptor.resting_potential)/
                                                   electroreceptor.resistance
  # channel current
  electroreceptor.current[] = (electroreceptor.potential[]-electroreceptor.reversal_potential)*
                                                                 conductance

  # receptor potential
  electroreceptor.potential[] -= dt*(leak_current +electroreceptor.current[])/electroreceptor.capacitance
end

# Construct a hair cell
electroreceptor = ElectroReceptor(0.0)
...
electroreceptor_animation_layout= GridLayout(3,3,
                colsizes = [Relative(.025), Relative(.95), Relative(.025)],
                rowsizes = [Relative(.025), Relative(.025), Relative(.95)])
electroreceptor_animation_layout[3, 1:3] = hc_animation_axis = LAxis(scene)
...
electroreceptor_animation_layout[2,2] = kinocilium_slider
electroreceptor_animation_layout[1, 2] = LText(scene,
        "HAIR CELL", textsize = 14)
electroreceptor_animation_layout[2, 1] = LText(scene, "Δk", textsize = 20)
controlpanel_layout[1,1] = electroreceptor_animation_layout
...
receptor_potential_trace = fill(electroreceptor_resting_potential, numt)
lines!(receptor_potential_axis,
       t, 1000.0*electroreceptor_resting_potential*ones(numt), color = :darkred)
       #
       # display(scene)
       # sleep(5000)
...
@async while isopen(scene) # run this block as parallel thread
  ...
  electroreceptor_stateupdate!(electroreceptor, kinocilium_slider.value[])
  threshold = 250.
  ...
  if spike
    μ = (1.0/3000.0)/(electroreceptor.potential[] - electroreceptor.resting_potential)
  ...
  # update display
  movegon(kinocilium_handle, 1, kcx0+electroreceptor.Δk[]*hairScale/nano, kcy0)
  movegon(tracker_handle, 1, electroreceptor.Δk[]/nano, electroreceptor.p_open[])

  # gate marker is gold if open, blue if closed
  channel_handle[:color] =
        @inbounds [electroreceptor.gateOpen[i] ? open_color : closed_color for i in 1:48]

  # shift display buffers left, insert new values on right
  shiftinsert(receptor_current_trace, electroreceptor.current[]/nano )
  shiftinsert(receptor_potential_trace, Float32(1000.0*electroreceptor.potential[]) )
